...
:: %1 function to be searched for
:searchToFile
  ECHO Saving result(s) in %fileDirectory%\%result%.txt
  ::if the file already exists then delete it
	IF EXIST %fileDirectory%\%result%.txt del %fileDirectory%\%result%.txt
	FOR /f %%i IN ('findstr /M /S %function% %searchDirectory%') DO CALL :loopBody %function% %%i >> %fileDirectory%\%result%.txt
	PAUSE
	EXIT
